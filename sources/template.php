<?php
    ...
    $d->reset();
    $d->query("select diachi,dienthoai,email,web  from #_company where type='footer'  limit 0,1");
    ...
    $d->reset();
    $d->query("select ten_$lang as ten,dienthoai,email from #_yahoo where hienthi=1 order by id asc");
    $hotro = $d->result_array();

    $d->reset();
    $d->query("select ten_$lang as ten,tenkhongdau,id from #_product_list where  type='dichvu' and hienthi=1 order by stt,id desc");
    $list = $d->result_array();
