...
function sessionhash(){
	...
	$token = md5(time());
	$cookiehash = md5(sha1($row['password'].$username));
...
function check_admin_index(){
	...
	if(isset($_SESSION[$login_name]) || $_SESSION[$login_name]==true){
		...
		if($_SESSION['login_session']!=$cookiehash || ($timenow - $row['lastlogin'])>3600 ) {
			cancel_session_admin($login_name);	
...
function cancel_session_admin($login_name){
...
function encrypt_password($token,$str,$salt){return md5(sha1($token.$str.$salt));}
...
function count_online(){
	...
	$d->query("select id,session_id from #_online order by id desc");
	...
	while(($rows[$i]['session_id'] != $ssid) && $i++<$result['dangxem']);
		...
		if($i<$result['dangxem']){
			$data_online['time'] = time();
			$d->reset();
			$d->setTable("online");
			$d->setWhere("session_id",$ssid);
			$d->update($data_online);
			$result['daxem'] = $rows[0]['id'];
		}
		else{
			$data_online['session_id'] = $ssid;
			$data_online['time'] = time();
			$d->reset();
			$d->setTable("online");
			$insertID = $d->insert($data_online);
			$result['daxem'] = $insertID;
			$result['dangxem']++;
		}
		return $result;
}
