...
function save_photo(){
	...
	if($id){
		foreach ($config['lang'] as $key => $value) {
			if($photo = upload_image("file_".$key, 'Jpg|jpg|png|gif|JPG|jpeg|JPEG|jp2', _upload_hinhanh,$file_name.$key)){
		...
		$upload_image=false;
		foreach ($config['lang'] as $key => $value) {
			if($data['photo_'.$key] = upload_image("file_".$key, 'Jpg|jpg|png|gif|JPG|jpeg|JPEG|jp2', _upload_hinhanh,$file_name.$key)){		
